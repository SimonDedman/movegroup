...
myLocation <- st_bbox(lemons.ras) %>% as.vector() # # bounding box lowerleftlon, lowerleftlat, upperrightlon, upperrightlat
...
# # Error in st_as_sf.stars(lemons.ras, as_points = FALSE, merge = TRUE) : 
# # merge=TRUE and as_points=TRUE unsupported; consider using st_contour for generating contour lines
# lemons.sf <- st_as_sf(lemons.ras, as_points = FALSE, merge = FALSE)

mycontour <- st_contour(lemons.ras,
                        contour_lines = TRUE, #false = polygons, true = lines
                        # breaks = classInt::classIntervals(na.omit(as.vector(lemons.ras[[1]])))$brks
                        # numerical; values at which to "draw" contour levels
                        breaks = c(0.5, 0.95))

mycontour <- st_contour(x = lemons.ras)
mycontour2 <- st_contour(lemons.ras)
mycontour == mycontour2

class(mycontour) # "sf"         "data.frame"
mycontour$geometry[1]
st_geometry(mycontour)
st_geometry(crop_map)
range(lemons.ras$All_Rasters_Scaled.asc, na.rm = TRUE)
st_bbox(mycontour)
# xmin ymin xmax ymax 
#   NA   NA   NA   NA
st_bbox(lemons.ras)
...
ggplot() + geom_stars(data = lemons.ras)

tif = system.file("tif/L7_ETMs.tif", package = "stars") # just a character string location
x = read_stars(tif)[, 1:50, 1:50, 1:2] # original, 2 bands
x = read_stars(tif)[, 1:50, 1:50] # edit, 1 band
x[[1]] = round(x[[1]]/5)
l = st_contour(x,
               contour_lines = TRUE,
               breaks = 11:15)
...
range(lemons.ras$All_Rasters_Scaled.asc, na.rm = TRUE) # 0 1
lemons.ras$All_Rasters_Scaled.asc <- round(lemons.ras$All_Rasters_Scaled.asc * 100)
range(lemons.ras$All_Rasters_Scaled.asc, na.rm = TRUE) # 0 100
tmp <- as.data.frame(lemons.ras$All_Rasters_Scaled.asc) # looks fine, values present
dim(x)
# x    y band 
#   50   50    6 # 2 original, 6 if you remove 1:2
dim(lemons.ras)
 #  x   y 
 # 69 111 

dims_x <- st_dimensions(x)
dims_lem <- st_dimensions(lemons.ras)
length(dims_x) # 3
length(dims_lem) # 2
lemons.ras2 <- st_redimension(x = lemons.ras,
               new_dims = 1:3)
# Error in st_redimension.stars(x = lemons.ras, new_dims = 1:3) : 
#   product of dim(new_dim) does not match that of x
# Might not be possible to just add a band.
...
# All_Rasters_Scaled.asc  1.214989e-12 8.115487e-08 5.516034e-06 0.002278308 0.0002800115    1 4226
# dimension(s):
# curvilinear grid

lemons.spl = split(lemons.ras, "band")

# st_contour fail section ends####

ggplot() + 
# ggmap(myMap) +
  # geom_stars(data = sqrt(lemons.ras), inherit.aes = FALSE) + # geom_stars only works for objects with raster or vector geometries
  # inherit.aes=FALSE from https://github.com/r-spatial/sf/issues/336
  # geom_sf(st_contour(x = lemons.ras,
  #                    contour_lines = FALSE, #false = polygons, true = lines
  #                    # breaks = classInt::classIntervals(na.omit(as.vector(lemons.ras[[1]])))$brks
  #                    # numerical; values at which to "draw" contour levels
  #                    breaks = c(0.5, 0.95))) +
  geom_sf(mycontour) +
  ...
  # https://github.com/r-spatial/stars/issues/465 st_contour returns an sf object with the contours; you'd have to pass that to geom_sf()
...
#        plot = last_plot(), device = "png", path = "../../Maps & Surveys/R_Plot_Outputs", scale = 1.75,
