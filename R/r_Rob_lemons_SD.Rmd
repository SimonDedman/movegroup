...
if (grepl("aurits", Sys.info()["nodename"])) {
  data.dir <- "~/Documents/Science/Projects/Rob Bullock - Bimini/dBBMM_HomeRange_output"
}
...
### for the package i think it would be useful for the user to print which IDs get removed
```
...
for (i in unique(DET$Shark)) { # i <- unique(DET$Shark)[1]
  ...
  # does this mean each fish's data are scaled to 0:1??####
  # check this ####
  rm(tmp)
  ...
  # values are integers i.e. how many 1m2 cells qualify as having values >= 0.5 or 0.95
...
write.csv(md,
          file = file.path(data.dir, out.dir, "Lemons_VolumeArea_AbsoluteScale.csv"),
          row.names = FALSE
)
...
```{r calculate volume area - merge with chunk above when finished}
# retrieve IDs to loop over
namevec <- list.files(path = file.path(data.dir, out.dir, "Aggregated/Scaled"), pattern = "asc")
namevec <- namevec[-c(1,2)]
namevec <- gsub('.asc', '', namevec)

# Pull all raster names from path into a list
filelist <- as.list(list.files(path = file.path(data.dir, out.dir, "Aggregated/Scaled"), pattern = "asc"))

# Remove first 2 objects
filelist[c(1,2)] <- NULL
...
for (i in 1:length(rasterlist)){
  ...
  # Extract vector with non-zero values
  temp <- rasterlist[[i]][which(rasterlist[[i]][] != 0)]
  
  # Extract maximum value
  max <- maxValue(rasterlist[[i]])
  
  # 50%
  area.50 <- length(which(rasterlist[[i]][] >= (max(temp)*.5)))
  area.50
  
  # 95%
  area.95 <- length(which(rasterlist[[i]][] >= (max(temp)*.05)))
  area.95
...
write.csv(md,
          file = file.path(data.dir, out.dir, "Aggregated/Scaled", "Lemons_VolumeArea_RelativeScale.csv"),
          row.names = FALSE
)
...
ggmap(myMap) +
  ...
  ggtitle(paste0("95% and 50% UD contours"),
          subtitle = paste0("Aggregated scaled contours. n = ", length(unique(check2$Shark)))) +
...
```{r construct_tidal_dbbmm and plot}
...
for (i in unique(DET$T.Ph)) { # "H" "M" "L"
  ...
  for (j in unique(DET.t$Shark)) {
    ...
    # Warning: Some burst are omitted for variance calculation since there are not segments of interest
  ...
  write.csv(
    x = md,
    file = file.path(data.dir, out.dir, "Tide", paste0("Lemons_VolumeArea_AbsoluteScale_", i, ".csv")),
    row.names = FALSE
  )
  ...
  # Calculate volume areas for scaled individual UDs
  
  # retrieve IDs to loop over
  namevec <- list.files(path = file.path(data.dir, out.dir, "Tide", i,"Scaled"), pattern = "asc")
  namevec <- namevec[-c(1,2)]
  namevec <- gsub('.asc', '', namevec)
  
  # Pull all raster names from path into a list
  filelist <- as.list(list.files(path = file.path(data.dir, out.dir, "Tide", i,"Scaled"), pattern = "asc"))
  ...
  for (j in 1:length(rasterlist)){
    ...
    # Extract vector with non-zero values
    temp <- rasterlist[[j]][which(rasterlist[[j]][] != 0)]
    
    # Extract maximum value
    max <- maxValue(rasterlist[[j]])
    
    # 50%
    area.50 <- length(which(rasterlist[[j]][] >= (max(temp)*.5)))
    area.50
    
    # 95%
    area.95 <- length(which(rasterlist[[j]][] >= (max(temp)*.05)))
    area.95
  ...
  write.csv(md,
            file = file.path(data.dir, out.dir, "Tide",paste0("Lemons_VolumeArea_RelativeScale_",i,".csv")),
            row.names = FALSE)
  ...
  myLocation <- NULL
  if (is.null(myLocation)) myLocation <- st_bbox(lemons.ras %>% st_transform(4326)) %>% as.vector() # xmin      ymin      xmax      ymax 
  myLocation <- c(-79.28970, 25.68331, -79.21143, 25.78939)
  ...
  # Automate width * height adjustments for different map extent / ratio
  # 6 (manually chosen width, below), divided by width range times by height ranges
  # Maintains ratio by scales height to width(6). Then *1.2 because it still wasn't perfect.
  # attr(myMap, "bb")[[4]] - attr(myMap, "bb")[[2]] # longitude, x, width, bind as 6
  # attr(myMap, "bb")[[3]] - attr(myMap, "bb")[[1]] # latitude, y, height
  autoheight <- (6 / (attr(myMap, "bb")[[4]] - attr(myMap, "bb")[[2]])) * (attr(myMap, "bb")[[3]] - attr(myMap, "bb")[[1]]) * 1.2
  ...
  ggmap(myMap) +
    ...
    geom_sf(data = st_contour(x = lemons.ras, contour_lines = TRUE, breaks = c(0.5)) %>%
              st_transform(3857), fill = NA, inherit.aes = FALSE, aes(colour = "UD_50_pct")) +
    scale_colour_manual(name = "Percent UD Contours", values = c(UD_95_pct = "red", UD_50_pct = "orange")) +
    ggtitle(paste0("95% and 50% UD contours, ", "Tide: ",i),
            subtitle = paste0("Aggregated scaled contours. n = ", length(unique(check2$Shark)))) +
    # make title & subtitle function params ####
  # Can use the term 'home range' when an animal can be detected wherever it goes i.e. using GPS, satellite or acoustic telemetry whereby it is known that acoustic receivers cover the entire home range of the study species. This term is problematic when applied to a passive acoustic telemetry setting where an array of non-overlapping receivers are used to assess local space use patterns i.e. the home range is bigger than the coverage by the acoustic array; put in Details ####
  # DET %>% distinct(Shark) %>% nrow() # 13
