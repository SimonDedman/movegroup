...
myLocation <- st_bbox(lemons.ras %>% st_transform(4326)) %>% as.vector() # xmin      ymin      xmax      ymax 
...
st_crs(mycontour)
st_crs(myMap) <- st_crs(mycontour) # Coordinate Reference System: NA
# Error in UseMethod("st_crs<-") : 
#   no applicable method for 'st_crs<-' applied to an object of class "c('ggmap', 'raster')"
mycontour_bbox <- st_bbox(mycontour)
...
# 25.64153 -79.32129 25.83945 -79.14551
crs(lemons.ras) # NA
st_crs(lemons.ras) # EPSG:2958 
st_crs(mycontour) # EPSG:2958 


# Extract the bounding box (in lat/lon) from the ggmap to a numeric vector, 
# and set the names to what sf::st_bbox expects:
map_bbox <- setNames(unlist(attr(myMap, "bb")), 
                     c("ymin", "xmin", "ymax", "xmax"))
# Coonvert the bbox to an sf polygon, transform it to 3857, 
# and convert back to a bbox (convoluted, but it works)
bbox_3857 <- st_bbox(st_transform(st_as_sfc(st_bbox(map_bbox, crs = 4326)), 3857))
# Overwrite the bbox of the ggmap object with the transformed coordinates 
attr(myMap, "bb")$ll.lat <- bbox_3857["ymin"]
attr(myMap, "bb")$ll.lon <- bbox_3857["xmin"]
attr(myMap, "bb")$ur.lat <- bbox_3857["ymax"]
attr(myMap, "bb")$ur.lon <- bbox_3857["xmax"]

plot(st_transform(mycontour, crs = 4326)[1], bgMap = myMap)
...
ggmap(myMap) +
  ...
  # geom_stars only works for objects with raster or vector geometries
  # inherit.aes=FALSE from https://github.com/r-spatial/sf/issues/336
  # Coordinate system already present. Adding new coordinate system, which will replace the existing one.
  # Get this error when using st_transform but not if not, so myMap presumably has same crs as lemons.ras i.e.
  # 2958. st_transform(2958) plots but no data.
  
  # geom_sf(data = mycontour %>% st_transform(4326),
  geom_sf(data = mycontour,
          fill = NA,
          color = "orange") +
  # Coordinate system already present. Adding new coordinate system, which will replace the existing one.
  # Error in FUN(X[[i]], ...) : object 'lon' not found
  # https://github.com/dkahle/ggmap/issues/160 # main bug, posted there.
  # https://github.com/dkahle/ggmap/pull/162 # tried below, no success
  # https://github.com/r-spatial/stars/issues/465
  
  # coord_sf(crs = st_crs(2958)) + # 3857 4326
  # coord_sf(xlim = c(mycontour_bbox["xmin"] - .1 * (mycontour_bbox["xmax"] - mycontour_bbox["xmin"]),
  #                 mycontour_bbox["xmax"] + .1 * (mycontour_bbox["xmax"] - mycontour_bbox["xmin"])),
  #        ylim = c(mycontour_bbox["ymin"] - .1 * (mycontour_bbox["ymax"] - mycontour_bbox["ymin"]),
  #                 mycontour_bbox["ymax"] + .1 * (mycontour_bbox["ymax"] - mycontour_bbox["ymin"]))) +
