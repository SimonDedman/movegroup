...
for (i in unique(DET$Shark)) { # i <- unique(DET$Shark)[1]
  ...
  # Print individual
  counter <- counter + 1
  print(paste0("processing ", which(unique(DET$Shark) %in% i),
  ...
  buffpct <- buffpct / 100
  xUTM.i <- raster(xmn = min(DET$NewEastingUTM, na.rm = TRUE) - ((max(DET$NewEastingUTM, na.rm = TRUE) - min(DET$NewEastingUTM, na.rm = TRUE)) * buffpct),
  ...
  x.i <- raster(xmn = min(DET$NewEastingUTM),
...
for (i in unique(DET$T.Ph)) { # "H" "M" "L"
  ...
  for (j in unique(DET.t$Shark)) {
    # Print individual
    counter <- counter + 1
    print(paste0("processing ", which(unique(DET.t$Shark) %in% j),
