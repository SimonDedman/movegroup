...
dBBMM_HomeRange <- function(
  ...
  Lon = NULL,
  Group = NULL, # name of grouping column in data
  ...
  # Unless I can rename the user entry to "ID"
  # Same for Lat & Lon
...
# A dBBMM is nor run if total detections of individual \< window size (default value, 31).
...
for (i in unique(data$ID)) { # i <- unique(data$ID)[1]
  ...
  move.i$LocationError <- moveLocError # 1 m: gps loc of shark inferred from boat coord + bearing + distance estimate
  ...
  bursted_dbbmm <- brownian.bridge.dyn(bursted,
                                       burstType = "normal",
                                       raster = xAEQD.i,
                                       location.error = bbdlocationerror,
                                       ext = bbdext,
...
md <- bind_rows(bb.list,
  ...
  select(!column_label) # remove column_label column
...
scaleraster(path = file.path(data.dir, out.dir))
# ```
...
lemons.ras <- read_stars(paste0(data.dir, "/dBBMM ASCII/Scaled/All_Rasters_Scaled.asc")) %>% 
  # si directory
  st_set_crs(2958)
...
ggsave(paste0(today(), "_dBBMM-contours.png"),
       plot = last_plot(), device = "png", path = file.path(work.dir, out.dir, "Scaled"), scale = 1,
...
lemons.ras <- read_stars(file.path(data.dir, out.dir, "Tide", i, "Scaled/All_Rasters_Scaled.asc")) %>% 
  # si directory
  st_set_crs(2958)
# stars:::is_curvilinear(lemons.ras) # FALSE
