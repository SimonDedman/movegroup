...
move.sp <- move(
  x = df.new$Lon, y = df.new$Lat, time = df.new$Datetime,
...
e <- 12000
xUTM.sp <- raster(
  xmn = min(df.new$NewEastingUTM) - e, xmx = max(df.new$NewEastingUTM) + e, ymn = min(df.new$NewNorthingUTM) - e, ymx = max(df.new$NewNorthingUTM) + e,
...
x.sp <- raster(
  xmn = min(df.new$NewEastingUTM), xmx = max(df.new$NewEastingUTM), ymn = min(df.new$NewNorthingUTM), ymx = max(df.new$NewNorthingUTM),
